...
structure of a BT is like a directed rooted tree with internal nodes called control flow nodes and leaf nodes called 
execution nodes. Each connected node are most often called parent and child where the root is the node without parents. 
The execution of the tree starts with the root that sends signals to its children to start executing. The cild then 
returns running when the execution is under way and then success or failure depending on whether the process could complete 
or not. In this way the flow of tasks can be controlled\cite{BTsinAI}. 
